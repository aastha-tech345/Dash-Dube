...
export default function CreateZone() {
  ...
  const fetchFloors = async (warehouseId: number) => {
    try {
      ...
      // If no floors exist, create a default floor entry for the form
      if (data.length === 0) {
        console.log('No floors found for warehouse, using default floor');
        ...
        // Auto-select the default floor
        if (formData.floorId === 0) {
          setFormData(prev => ({ ...prev, floorId: 1 }));
        }
      }
    } catch (error) {
      console.error('Failed to fetch floors:', error);
      
      // Create a default floor entry even on API error
      const defaultFloor: FloorResponse = { 
        id: 1, 
        warehouseId, 
        warehouseName: '', 
        code: 'FL-01', 
        name: 'Ground Floor', 
        description: 'Default floor - API unavailable',
        levelNo: 1,
        length: 0,
        width: 0,
        height: 0,
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      console.log(`${isEditMode ? 'Updating' : 'Creating'} zone data:`, formData);

      let result;
      if (isEditMode && editId) {
        // Update existing zone
        result = await warehouseApi.updateZone(parseInt(editId), formData);
        ...
        // Create new zone
        result = await warehouseApi.createZone(formData);
